...
def fetch_all_data():
    ...
    # 新北市 (增加重試機制)
    try:
        n_res = requests.get("https://data.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/json", timeout=15).json()
        for r in n_res:
...
if 'addr_label' not in st.session_state: st.session_state['addr_label'] = "讀取位置中..."

curr = get_geolocation()
if curr and 'coords' in curr:
    n_lat, n_lon = round(curr['coords']['latitude'], 4), round(curr['coords']['longitude'], 4)
    if abs(n_lat - st.session_state['gps_pos'][0]) > 0.0005 or st.session_state['addr_label'] == "讀取位置中...":
        st.session_state['gps_pos'] = (n_lat, n_lon)
        new_addr = get_addr_pro(n_lat, n_lon)
        st.session_state['addr_label'] = new_addr if new_addr else f"{n_lat}, {n_lon}"

u_lat, u_lon = st.session_state['gps_pos']

# --- 5. 畫面渲染 ---
st.title("🛡️ 雙北全域戰情室 (行政區+選單版)")
df = fetch_all_data()
...
m4.metric("目前地址", st.session_state['addr_label'])
...
with col_map:
    ...
    if not df.empty:
        for _, row in df.iterrows():
            color = '#d32f2f' if row['佔用%'] >= 90 else ('#ffa500' if row['佔用%'] >= 75 else '#388e3c')
    ...
    st_folium(m, width="100%", height=550, key="final_dashboard_map")
