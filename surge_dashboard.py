...
def fetch_complete_data():
    ...
    # --- 新北市部分 (核心救援：CSV 與 opendata 子網域備援) ---
    ntp_sources = [
        ("CSV備援", "https://data.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/csv?size=1000"),
        ("備用網域", "https://opendata.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/json?size=1000")
    ]
    ...
    for label, url in ntp_sources:
        ...
        try:
                    ...
                    for _, r in n_df.iterrows():
                        try:
                            ...
                            if 24.5 < lat < 25.5:
                                ...
                                all_data.append({'場站名稱': r['NAME'], 'lat': lat, 'lon': lon, '佔用%': occ, '行政區': r['AREA'], '縣市': '新北'})
                        except: continue
                    ntp_success = True
                else:
                    # JSON 備用網域處理邏輯
            ...
            st.session_state['ntp_err'] = f"{label}失敗: {str(e)[:20]}"
...
if ntp_count == 0:
    st.caption(f"⚠️ 偵錯狀態: {st.session_state.get('ntp_err', '等待通訊中')}")
...
with col_map:
    ...
    st_folium(m, width="100%", height=600, key="uber_radar_v_rescue")
