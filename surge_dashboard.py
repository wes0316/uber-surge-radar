...
st.markdown("""
    <style>
        ...
        [data-testid="stToggle"] div { background-color: #2D1B1B !important; border: 2px solid #8B4513 !important; }
        [data-testid="stToggle"] input:checked + div { background-color: #00D4FF !important; border: 2px solid #00D4FF !important; box-shadow: 0 0 30px rgba(0, 212, 255, 1) !important; }
        [data-testid="stToggle"] div div { background-color: #FF4444 !important; border: 2px solid #CC0000 !important; }
        [data-testid="stToggle"] input:checked + div div { background-color: #00FF88 !important; border: 2px solid #00CC66 !important; }
...
with st.sidebar:
    ...
    # 修改按鈕邏輯：點擊即清空快取，無需 st.rerun (Streamlit 原生就會重跑)
    if st.button("🔄 立即重新整理"):
        st.cache_data.clear()
...
with col_map:
    ...
    st_folium(m, width="100%", height=580, key=f"v8_{show_rain}_{show_heatmap}_{zoom}")
...
# --- 8. 非阻塞式前端自動刷新 (取代死當的 time.sleep) ---
components.html("""
    <script>
        // 設定一個 180 秒的計時器，時間到時自動尋找並點擊側邊欄的按鈕
        setTimeout(function() {
