            ...
            if pid in dyn_map:
                ...
                if tw97x and tw97y and total > 0 and avail >= 0:
                    try:
                        ...
                        all_data.append({'場站名稱': s.get('NAME', '未知站點'), 'lat': lat, 'lon': lon, '佔用%': round(max(0, min(100, occ)), 1), '行政區': dist_name, '縣市': '新北'})
                    except: pass
    except: pass
...
with st.sidebar:
    ...
    st.markdown("### 📍 圖例說明")
    st.markdown(f"""
        <div style="margin-bottom: 5px;"><span style="color:#FF0000; font-weight:bold;">●</span><span style="color:#DCDCDC; margin-left:10px;">站點紅區 (>= 90%)</span></div>
        <div style="margin-bottom: 5px;"><span style="color:#FFAA00; font-weight:bold;">●</span><span style="color:#DCDCDC; margin-left:10px;">站點高潛力 (75-89%)</span></div>
...
m4.metric("座標位置", f"{st.session_state['gps_pos']}")
