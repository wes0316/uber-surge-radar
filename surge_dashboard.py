...
if curr and 'coords' in curr:
    ...
    if abs(n_lat - st.session_state['gps_pos'][0]) > 0.0005 or st.session_state['addr_label'] == "正在定位...":
        ...
        new_addr = get_address_pro(n_lat, n_lon)
        st.session_state['addr_label'] = new_addr if new_addr else f"{n_lat}, {n_lon}"

u_lat, u_lon = st.session_state['gps_pos']

# --- 5. UI 渲染 (修正統計數字 Bug) ---
st.header("🛡️ 雙北全域戰情室 (完全修復版)")
df = fetch_complete_data()

# 核心修正：直接從 df 算數量，保證不為 0
count_tpe = len(df[df['縣市'] == '台北']) if not df.empty else 0
count_ntpc = len(df[df['縣市'] == '新北']) if not df.empty else 0
count_surge = len(df[df['佔用%'] >= 90]) if not df.empty else 0

m1, m2, m3, m4 = st.columns(4)
m1.metric("台北站點", f"{count_tpe} 處")
m2.metric("新北站點", f"{count_ntpc} 處")
m3.metric("Surge 警戒點", f"{count_surge} 處")
...
with col_map:
    # 使用 Google 亮色瓦片
    m = folium.Map(location=[u_lat, u_lon], zoom_start=zoom_val, 
    ...
    st_folium(m, width="100%", height=550, key="final_fix_map")
