...
def get_address_pro(lat, lon):
    """將座標轉換為中文路名 (Reverse Geocoding)"""
    try:
        # 使用隨機 User-Agent 避免被 Nominatim 擋掉
        ...
        return f"{lat}, {lon}" # 失敗時暫時回傳座標作為備案
...
# --- 5. UI 渲染 ---
st.header("🛡️ 雙北全域戰情室 (地址路名版)")
df, stats = fetch_complete_data()
...
m4.metric("目前位置", st.session_state['addr_label']) # 重點：座標轉路名
...
with col_map:
    ...
    st_folium(m, width="100%", height=550, key="addr_v14_map")
