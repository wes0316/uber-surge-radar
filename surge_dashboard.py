...
st.markdown("""
    <style>
        ...
        div[data-testid="stMetric"] { background-color: #242424 !important; border-left: 5px solid #276EF1 !important; }
...
def fetch_complete_data():
    ...
    t_now = int(time.time())
    ntp_sources = [
        ("主要入口", f"https://data.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/json?size=1000&v={t_now}"),
        ("CSV入口", f"https://data.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/csv?size=1000&v={t_now}"),
        ("備援網域", f"https://opendata.ntpc.gov.tw/api/datasets/E09B3532-60D6-4547-BE9A-60C1F7AA0B0A/json?size=1000")
    ]
    
    ntp_success = False
    for label, url in ntp_sources:
        if ntp_success: break
        try:
            res = requests.get(url, headers=headers, timeout=15, verify=False)
            if res.status_code == 200:
                if "csv" in url:
                    n_df = pd.read_csv(io.StringIO(res.text))
                    n_df.columns = [c.upper() for c in n_df.columns]
                    for _, r in n_df.iterrows():
                        lat, lon = float(r['LAT']), float(r['LON'])
                        if 24.5 < lat < 25.5:
                            tot, ava = float(r['TOTAL']), float(r['AVAILABLE'])
                            all_data.append({'場站名稱': r['NAME'], 'lat': lat, 'lon': lon, '佔用%': round(max(0, min(100, ((tot-ava)/tot*100))), 1), '行政區': r['AREA'], '縣市': '新北'})
                else:
                    n_res = res.json()
                    for r in n_res:
                        lat, lon = float(r.get('LAT') or r.get('lat', 0)), float(r.get('LON') or r.get('lon', 0))
                        if 24.5 < lat < 25.5:
                            tot, ava = float(r.get('TOTAL') or 1), float(r.get('AVAILABLE') or 0)
                            all_data.append({'場站名稱': r.get('NAME'), 'lat': lat, 'lon': lon, '佔用%': round(max(0, min(100, ((tot-ava)/tot*100))), 1), '行政區': r.get('AREA'), '縣市': '新北'})
                ntp_success = True
            else:
                st.session_state['ntp_err'] = f"{label}報錯: HTTP {res.status_code}"
        except Exception as e:
            st.session_state['ntp_err'] = f"{label}異常: {str(e)[:15]}"
...
# 診斷標籤優化
if ntp_count == 0:
    st.caption(f"⚠️ 偵錯狀態: {st.session_state.get('ntp_err', '所有路徑均被封鎖')}")
...
with col_map:
    ...
    st_folium(m, width="100%", height=600, key="uber_radar_rescue_v2")
